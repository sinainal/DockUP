...
def run_e2e() -> None:
    ...
    step(
        f"Selected native ligand for grid: {main_ligand_label} (chain {main_ligand_chain})"
    )
    ...
    step(
        "Computed grid center="
        f"({grid['cx']:.3f}, {grid['cy']:.3f}, {grid['cz']:.3f}) size=20x20x20"
    )
    ...
    convert_resp = api(
        ...
        "/ligand-3d/api/convert3d",
        {
            "smiles": smiles,
            "name": "ethylene",
            "file_stem": f"ethylene_dock_test_{run_stamp}",
        },
        timeout=180,
    )
    ...
    queue_resp = api(
        ...
        "/api/queue/build",
        {
            ...
            "selection_map": {
                receptor_id: {
                    "chain": main_ligand_chain,
                    "ligand_resname": ligand_filename,
                }
            },
    ...
    if added != 1:
        raise RuntimeError(
            f"Expected exactly 1 new queue job, got {added}. queue/build debug={debug}"
        )
